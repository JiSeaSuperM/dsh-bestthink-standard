 ...
 * comments, constants, and function names are preserved; this port adds the
 * configurable keyword extension (`extraReactKeywords` / `extraSpecKeywords`
 * accepted as the second argument of `classifyTask` and `sessionMode`), so a
 * deployment can widen the classifier to its own task domain without editing
 * this file.
 *
...
/** Extend a base regex with user keywords (empty list keeps the base). */
function withExtraKeywords(base, keywords) {
  if (!Array.isArray(keywords) || keywords.length === 0) return base
  const extra = keywords.map(escapeRegExp).join('|')
  return new RegExp(`${base.source}|${extra}`, base.flags)
}
 ...
 * @param extras - optional { extraReactKeywords?, extraSpecKeywords? } lists
 *   that widen the built-in keyword sets (defaults: empty = pure generic).
 */
export function classifyTask(text, extras) {
  const reactRe = withExtraKeywords(REACT_RE, extras?.extraReactKeywords)
  const specRe = withExtraKeywords(SPEC_RE, extras?.extraSpecKeywords)
  const react = countHits(reactRe, text)
  const spec = countHits(specRe, text)
...
export function sessionMode(session, extras) {
  const events = session.events
  const userMsg = events.find((e) => e.type === 'user/message')
  return classifyTask(extractText(userMsg?.data), extras)
}
